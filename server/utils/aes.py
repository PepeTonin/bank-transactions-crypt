...
def decryptAes(key: bytes, iv: bytes, ciphertext: bytes) -> bytes:
    """
...
def encryptAes(key: bytes, iv: bytes, plaintext: bytes) -> bytes:
    """
...
def useDecryptAes(data: str, iv: str, key: str):
    data = bytes.fromhex(data)
    iv = bytes.fromhex(iv)
    key = bytes.fromhex(key)
    decrypted_data = decryptAes(key, iv, data)
...
if __name__ == "__main__":
    ...
    decrypted_data = decryptAes(key, iv, ciphertext)
